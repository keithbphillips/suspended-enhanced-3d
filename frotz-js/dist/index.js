...
var DFrotzInterface = function () {
	...
	_createClass(DFrotzInterface, [{
		...
		value: function restoreSave(saveFileExists) {
			...
			if (saveFileExists) {
				// Use relative path for restore since dfrotz runs in the save directory
				var relativeSaveFile = path.basename(this.saveFile);

				// Check file before restore
				toResolve = [this.command('restore'), this.command(relativeSaveFile)];
			} else {
		...
		value: function writeSave() {
			...
			// Use relative path for save file since dfrotz runs in the save directory
			var relativeSaveFile = path.basename(this.saveFile);

			var toResolve = [this.command('save'), this.command(relativeSaveFile), this.command('Y'), this.command('quit'), this.command('Y'), this.command('SI')];
		...
		value: function init(cb) {
			...
			// Set working directory to the directory containing the save file
			var saveDir = path.dirname(this.saveFile);

			this.dfrotz = childProcess.execFile(this.executable, dfrotzArgs, {
				encoding: 'utf8',
				cwd: saveDir
